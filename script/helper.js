...
export const setNavBar = (isSearchNeeded = true) => {
  ...
  const searchBar = ` <div class="search-wrapper">
        <select class="filter-menu" name="category" id="filter-menu"></select>
        <div class="search-bar">
          <input type="text" id="search" placeholder="Search some thing.." />
          <img src="./assets/images/search.png" alt="" />
        </div>
      </div>`;
  const navbarItem = `<h1 class="logo" id="logo">MI Store</h1>
  ${isSearchNeeded ? searchBar : ""}  
      ...
      ${
       ...
       <span id="login-status">${checkLogin() ? "Logout" : " Login"}</span
        >
      </div>`;
...
export const isLogdedIn = () => {
  ...
  !isLogdedIn ? (window.location = "../Login.html") : null;
};
